<template>
  <Transition>
    ...
    >
      ...
      <slot :meta="resourceMeta" name="tableWrapper">
        <TableWrapper>
          <slot name="table">
            <table
              :class="{ 'table-striped': striped }"
              class="table inertia-table table-hover"
            >
              <!-- Table Head -->
              <thead>
                <slot
...
.inertia-table {
  background-color: var(--inertia-table-background-color);
  color: var(--inertia-table-color);
}

.inertia-table-direction-icon {
  width: 1rem;
  height: 1rem;
}
</style>